async function deleteMsisdnClient() {
...
const token = document.getElementById('token').value;
   if (!token) {
    ...
    try {
        const response = await fetch(`http://172.17.0.1:8100/provision/v0/delete/msisdn/${msisdn}`, {
            method: 'DELETE',
           headers: {
...
async function deleteEmailClient() {
...
const token = document.getElementById('token').value;
   if (!token) {
    ...
    try {
        const response = await fetch(`http://172.17.0.1:8100/provision/v0/delete/email/${email}`, {
            method: 'DELETE',
           headers: {
               'Authorization': `Bearer ${token}`
           }
...
async function deleteDniClient() {
...
const token = document.getElementById('token').value;
   if (!token) {
    ...
    try {
        const response = await fetch(`http://172.17.0.1:8100/provision/v0/delete/dni/${dni}`, {
            method: 'DELETE',
           headers: {
               'Authorization': `Bearer ${token}`
           }
