...
//    const token = await obtenerToken(); // OJO la configuración de Keycloack.
   ...
   console.log('Authorization header:', `Bearer ${token}`);
    try {
        const response = await fetch(`http://172.17.0.1:8100/provision/v0/get/msisdn/${msisdn}`, {
           headers: {
...
//
   ...
   if (!token) {
    ...
    try {
        const response = await fetch(`http://172.17.0.1:8100/provision/v0/get/email/${email}`, {
           headers: {
               'Authorization': `Bearer ${token}`
           }
        });
...
//
   ...
   if (!token) {
    ...
    try {
        const response = await fetch(`http://172.17.0.1:8100/provision/v0/get/dni/${dni}`, {
           headers: {
               'Authorization': `Bearer ${token}`
           }
        });
